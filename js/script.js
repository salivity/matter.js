...
document.addEventListener("DOMContentLoaded", () => {
    ...
    videos.forEach(video => {
        video.setAttribute("poster", "/matter.js/images/video/poster/default.svg")
    });
    ...
    const remapArticles = (data) => {
        return data.map((item) => {
            return { id: item[0], title: item[1], description: item[2], url: `/matter.js/article/${kebabCase(item[1])}`, open_graph: item[3] };
        });
    };
    ...
    if (articlesContainer) {
        fetch("/matter.js/js/articles.json").then(res => res.json()).then(data => {
    ...
    function renderList(items, container, paginatorId, page, isSearch, onPageChange) {
        ...
        pageItems.forEach(item => {
            ...
            if (item.open_graph === true) {
                ...
                imageTop.style.backgroundImage = `url(/matter.js/images/open-graph/article/${item.id}.webp)`;
                ...
                imageBottom.src = `/matter.js/images/open-graph/article/${item.id}.webp`;
